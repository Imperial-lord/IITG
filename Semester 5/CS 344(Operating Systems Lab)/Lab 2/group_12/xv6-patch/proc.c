...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(p!=0){
      ...
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      // if(p > ptable.proc && p < &ptable.proc[NPROC-1])
      //   cprintf("Process = %d   %d  %d\n",(p-1)->pid,p->pid,(p+1)->pid);
      //cprintf("Process = %d       %d\n",p->pid,p->burstTime);
      c->proc = p;
      switchuvm(p);
      p->state = RUNNING;
...
sys_get_burst_time(){
  int burstTime = -1;
  acquire(&ptable.lock);
  burstTime = mycpu()->proc->burstTime;
  release(&ptable.lock);
  return burstTime;
}