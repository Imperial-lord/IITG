...
# Plots for BSE - For plots look at Plots/Question 3/BSE/
...
for i in range(columnHeaders.shape[0]):
    ...
    for j in range(3):
        ...
        plt.xlabel('Normalised Log Returns - {} basis'.format(intervals[j]))
        plt.ylabel('Frequency of Normalised Log Returns')
        plt.title('Histogram of Normalised Log Returns ({}) for BSE - {}'.format(
            intervals[j], header))
        plt.legend()
        # plt.savefig(
        #     'Plots/Question 3/BSE/{}/{}.png'.format(intervals[j], header))
        plt.show()


# Plots for NSE - For plots look at Plots/Question 3/NSE/
...
for i in range(columnHeaders.shape[0]):
    ...
    for j in range(3):
        ...
        plt.xlabel('Normalised Log Returns - {} basis'.format(intervals[j]))
        plt.ylabel('Frequency of Normalised Log Returns')
        plt.title('Histogram of Normalised Log Returns ({}) for NSE - {}'.format(
            intervals[j], header))
        plt.legend()
        # plt.savefig(
        #     'Plots/Question 3/NSE/{}/{}.png'.format(intervals[j], header))
        plt.show()
